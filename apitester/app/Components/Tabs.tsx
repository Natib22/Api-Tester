...
const Tabs = () => {
  ...
  return (
    <div className=' h-8  my-2 flex items-center overflow-hidden '>
      <div className='w-[90%] h-full overflow-scroll flex items-center gap-2'>
      {tabs.map((tab, index) => (
        <div key = {index} className='flex h-full  min-w-1/3 w-auto max-w-52 p-1  items-center bg-[#2d2b2b] rounded-lg'> <span className='text-[10px] ml-1'>{tab.method}</span> <span className='text-xs m-1 overflow-hidden
